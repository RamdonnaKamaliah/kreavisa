    ...
    <div
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-8">
            <div class="flex flex-col sm:flex-row items-center gap-6">
                ...
                <div class="w-28 h-28 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-blue-500 shadow-lg">
                    @php
                        ...
                        $photoPath = !empty($user->foto)
                            ...
                            : '';
                        $photoUrl =
                            !empty($photoPath) && file_exists(public_path($photoPath))
                                ? asset($photoPath)
                                : $defaultPhoto;
                        $photoUrl .= '?v=' . time();
                    @endphp
                ...
                <div class="text-center sm:text-left">
                    <h1 class="text-2xl md:text-3xl font-bold text-black dark:text-white">{{ $user->nama_lengkap }}</h1>
                    <p class="text-gray-600 dark:text-gray-300">{{ $user->email }}</p>
                    <p class="text-sm text-amber-600 dark:text-amber-400 mt-1 font-semibold">Karyawan Navisa Basic
                        Collection</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-10">
            ...
            <div>
                ...
                <div class="bg-gray-100 dark:bg-slate-800 p-4 rounded-lg shadow-inner">
                    {{ $user->jabatan->nama_jabatan ?? 'Belum ada jabatan' }}</div>
            </div>
            <div>
            ...
            <div>
                ...
                <div class="bg-gray-100 dark:bg-slate-800 p-4 rounded-lg shadow-inner">
                    {{ \Carbon\Carbon::parse($user->tanggal_lahir)->format('d-m-Y') }}</div>
            </div>
